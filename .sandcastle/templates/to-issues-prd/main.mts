...
for (let i = 0; i < slices.length; i++) {
  ...
  try {
    ...
    console.error(`Created so far: ${createdNumbers.map((n) => `#${n}`).join(", ") || "(none)"}`);
    throw err;
  }

  const urlMatch = createOutput.match(/\/issues\/(\d+)\s*$/);
  if (!urlMatch) {
    console.error(`Could not parse issue number from \`gh issue create\` output: ${createOutput}`);
    process.exit(1);
  }
  ...
  execFileSync(
    "gh",
    ["api", "-X", "POST", `repos/${GH_REPO}/issues/${PRD_NUMBER}/sub_issues`, "-f", `sub_issue_id=${subIssueId}`],
    { encoding: "utf8" }
  );
